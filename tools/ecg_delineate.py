...
def p_bounds(mag: np.ndarray, qrs_on: int, fs: int,
    ...
    # Ноль — самое тихое место окна, отрезок TP: там предсердия ещё молчат.
    floor = float(np.percentile(mag[lo:hi], 15))
    top = float(np.max(mag[lo:hi]))
    if top < floor + P_MIN_MV:
        return None                    # зубца P не видно: мерцание, узловой ритм

    # Берём ПОСЛЕДНИЙ заметный горб перед комплексом, а не самый высокий: зубец
    # P — всегда последнее событие перед деполяризацией желудочков, а выше него
    # запросто оказывается артефакт оцифровки.
    thr_pk = floor + 0.45 * (top - floor)
    cand = list(lo + find_peaks(mag[lo:hi], height=thr_pk)[0])
    if mag[hi - 1] >= thr_pk:          # зубец P вплотную к комплексу: его горб
        cand.append(hi - 1)            # срезан краем окна и максимумом не будет
    if not cand:
        return None
    peak_i = int(cand[-1])
    ...
    hold = _ms(10, fs)
    on = _walk(mag[:hi], peak_i, thr, -1, hold)
    below = np.flatnonzero(mag[peak_i:hi] < thr)
...
P_MIN_MV = 0.04                 # ниже 0.4 мм на плёнке зубец P не различим
