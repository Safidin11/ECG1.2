...
> Восстановление реальных фото пока грубее синтетики: вертикальные базовые линии
> отведений в ядре зашиты под пропорции синтетической страницы (`Y_SHIFT_RATIO`),
> а сегментация на низком разрешении даёт пробелы. Точное позиционирование
> отведений — задача **Фазы 3 (layout)**, замер качества — **Фазы 5**.
