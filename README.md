...
Решение — **layout-aware реконструкция**, встроенная в пайплайн:
- `segment` отдаёт СЫРУЮ маску nnU-Net (метки игнорируем, берём как «где чернила»);
- `layout` сам находит 4 строки (по проекции маски) и 4 колонки, назначает
  отведения по шаблону 3×4+ритм, режет калибровочный импульс, определяет мм/пиксель;
- `vectorize` по каждой клетке строит сигнал (средняя y трассы → мВ, изолиния к 0,
  интерполяция пропусков, медианный фильтр) и помечает **покрытие** каждого
  отведения (cov%) — честный индикатор надёжности.
