...
# Цена подгонки раскладки. 0.3 — «неуверенно», но выгрузка 12x1 из ЕМИАС с
# лишним блоком в кадре давала 2.25, и это уже не неуверенность, а промах:
# отведения расставлены наугад. Такое надо показывать красным.
MAX_LAYOUT_COST = 1.0
...
def quality_problems(q: dict | None, check: dict | None) -> list[dict]:
    ...
    if q:
        ...
        cost = q.get("layout_cost", 0)
        if cost > MAX_LAYOUT_COST:
            out.append({"level": "red",
                        "what": f"Формат не опознан (цена {cost:.2f} при пороге "
                                f"{MAX_LAYOUT_COST:.1f}).",
                        "why": "Отведения расставлены наугад, подписям верить нельзя. "
                               "Чаще всего причина — лишнее в кадре: рядом с плёнкой "
                               "попал блок с заключением или усреднёнными комплексами, "
                               "и он считается частью записи. Обрежьте снимок по самой "
                               "плёнке и выберите формат вручную."})
        elif cost > 0.3:
            out.append({"level": "warn",
                        "what": f"Формат опознан неуверенно (цена {cost:.2f}).",
                        "why": "Выберите формат вручную в списке выше."})

        # Калибровочный импульс против миллиметровки. Расхождение означает, что
        # масштаб взят неверно, а он линейно уходит и в милливольты, и в
        # миллисекунды: цифры выглядят обычно, но неправильны все разом.
        p = q.get("pulse")
        if p and p.get("applied"):
            r = p["ratio"]
            out.append({"level": "warn",
                        "what": f"Масштаб взят по калибровочному импульсу, а не по "
                                f"сетке: {p['px_per_mm']:.1f} пикселя на миллиметр "
                                f"вместо {p['px_per_mm'] * r:.1f}. Амплитуды "
                                f"пересчитаны в {1 / r:.1f} раза, длительность "
                                f"плёнки {p['seconds']:.1f} с вместо 10.",
                        "why": "Сетки на снимке нет или она не прочиталась, а движок "
                               "масштаб всё равно выдаёт — и он оказывается вымышленным. "
                               "Импульс надёжнее: его высота по определению 1 мВ, это "
                               "заявление самого кардиографа. Поправка уже применена, "
                               "числа ниже верные."})
        elif p and p.get("ratio") and not p.get("agrees", True):
            r = p["ratio"]
            out.append({"level": "red",
                        "what": f"Масштаб плёнки под вопросом: калибровочный импульс "
                                f"даёт {p['px_per_mm']:.1f} пикселя на миллиметр, "
                                f"сетка — {p['px_per_mm'] * r:.1f}. Расхождение "
                                f"в {max(r, 1 / r):.1f} раза.",
                        "why": "Импульс — это заявление самого кардиографа: его высота "
                               "по определению 1 мВ. Сетку движок ищет на бумаге, и "
                               "когда её нет (выгрузка из информационной системы, снимок "
                               "экрана), число всё равно выдаётся — но оно вымышленное. "
                               "Пока верить нельзя ни амплитудам, ни длительностям: "
                               f"и те и другие смещены примерно во столько же раз."})
